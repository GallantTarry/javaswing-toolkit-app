...
import shaoxia.Utils.GlassComboBox; // ✨ 引入你刚抽取的下拉框工具类
...
public class RenamerPanel extends BackgroundPanel {
    ...
    public RenamerPanel(MainLauncher parent) {
        ...
        // 2. 电子回单按钮：极简调用 + 设置38px高度
        receiptToggleBtn = new ActionButton("电子回单命名生成");
        receiptToggleBtn.setPreferredSize(new Dimension(0, 38));
        gbc.gridy = 1;
        centerLeftPanel.add(receiptToggleBtn, gbc);

        // 3. 应答文件夹按钮：极简调用 + 设置38px高度
        ...
        // ✨ 彻底瘦身：直接 new 全局 GlassComboBox 工具类！
    ...
    private void handleNormalDrop(DropTargetDropEvent dtde) {
        try {
            ...
            // 🚀 已删除原本这里的 JOptionPane.showMessageDialog(null, "处理成功！");
            // 现在它会静默完成重命名，不再打扰你。
        } catch (Exception ex) {
            ex.printStackTrace();
        }
