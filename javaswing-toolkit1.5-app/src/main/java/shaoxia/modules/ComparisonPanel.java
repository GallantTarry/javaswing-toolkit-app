...
public class ComparisonPanel extends BackgroundPanel {
    ...
    public ComparisonPanel(MainLauncher parent) {
        ...
        // --- Word 拖拽区 ---
        GlassDropPanel wordDropPanel = new GlassDropPanel();
        wordDropPanel.setPreferredSize(new Dimension(300, 150));
        wordLabel = new JLabel("【拖放 DOCX 文件】", JLabel.CENTER);
        wordLabel.setForeground(Color.WHITE);
        wordLabel.setFont(new Font("微软雅黑", Font.BOLD, 16));
        wordDropPanel.add(wordLabel, BorderLayout.CENTER);

        // --- PDF 拖拽区 ---
        GlassDropPanel pdfDropPanel = new GlassDropPanel();
        pdfDropPanel.setPreferredSize(new Dimension(300, 150));
        pdfLabel = new JLabel("【拖放 PDF 文件】", JLabel.CENTER);
        pdfLabel.setForeground(Color.WHITE);
        pdfLabel.setFont(new Font("微软雅黑", Font.BOLD, 16));
        pdfDropPanel.add(pdfLabel, BorderLayout.CENTER);

        // 绑定监听器 (区域绑定到 Panel，文字更新绑定到 Label)
        setupDND(wordDropPanel, true, wordLabel);
        setupDND(pdfDropPanel, false, pdfLabel);

        dropPanel.add(wordDropPanel);
        dropPanel.add(pdfDropPanel);

        // 4. 中部：日志区域 (替换为全局 GlassPanel)
        logArea = new JTextArea();
        logArea.setEditable(false);
        logArea.setFont(new Font("微软雅黑", Font.BOLD, 16));
        logArea.setForeground(Color.WHITE);
        logArea.setOpaque(false);
        logArea.setBackground(new Color(0, 0, 0, 0)); // 剥离底色
        logArea.setLineWrap(true);
        logArea.setBorder(new EmptyBorder(10, 15, 10, 15)); // 内部呼吸间距
        logArea.setText("您好！等待指令。\n");
        ...
        scrollPane.getViewport().setUI(new javax.swing.plaf.basic.BasicViewportUI());
        ...
        // ✨ 将纯净的滚动条装入极暗玻璃容器
        GlassPanel logGlassWrapper = new GlassPanel(new BorderLayout());
        logGlassWrapper.add(scrollPane, BorderLayout.CENTER);

        // 用一个带透明边距的面板包裹，产生物理隔离距离
        JPanel logContainer = new JPanel(new BorderLayout());
        logContainer.setOpaque(false);
        logContainer.setBorder(new EmptyBorder(40, 0, 40, 0)); // 与上下的物理距离
        logContainer.add(logGlassWrapper, BorderLayout.CENTER);

        // 5. 底部：圆弧实体按钮 (极简 ActionButton，自动加载全局黑雾色)
        JPanel btnPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 60, 0));
        btnPanel.setOpaque(false);

        ActionButton btnRun = new ActionButton("开始比对");
        ActionButton btnBack = new ActionButton("<< 返回菜单");
    ...
    private void executeCompare(String w, String p) {
        ...
        SwingUtilities.invokeLater(() -> {
            if (finalStatus) {
                JOptionPane.showMessageDialog(this, "成功！所有项目匹配无误。", "校对结果", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this, "错误！发现不匹配项。详情请查看日志。", "校对结果", JOptionPane.ERROR_MESSAGE);
            }
        });
    }
    ...
    private boolean checkResult(String w, String p, String lab) {
        ...
        log(lab + " -> Word:[" + w.trim() + "] 对比 PDF:[" + p.trim() + "] -> " + (match ? "匹配" : "冲突"));
        return match;
    }
    ...
    private boolean checkTimeResult(String wt, String pt, String lab) {
        ...
        log(lab + " -> Word:[" + nW + "] 对比 PDF:[" + nP + "] -> " + (match ? "匹配" : "冲突"));
        return match;
    }
    ...
    private void setupDND(Component dropTargetArea, boolean isWord, JLabel labelToUpdate) {
        new DropTarget(dropTargetArea, new DropTargetAdapter() {
            public void drop(DropTargetDropEvent dtde) {
                try {
                    ...
                    // 更新文字状态
                    labelToUpdate.setText("<html><center>已加载<br/>" + f.getName() + "</center></html>");
                    labelToUpdate.setForeground(Color.WHITE);
                } catch (Exception e) {}
            }
        });
    }
