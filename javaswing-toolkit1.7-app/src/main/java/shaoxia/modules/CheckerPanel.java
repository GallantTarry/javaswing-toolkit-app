...
public class CheckerPanel extends BackgroundPanel {
    ...
    // 模糊比对级别 (0: 精确, 1: 保留一位小数, 2: 个位, 3: 十位)
    ...
    public CheckerPanel(MainLauncher parent) {
        ...
        // --- 全新改造：右上角模糊比对小圆点指示灯 ---
        JPanel rightTop = new JPanel(new FlowLayout(FlowLayout.RIGHT, 0, 0)); // 消除边距
        rightTop.setOpaque(false);

        // 引入新的小圆点指示灯组件
        FuzzyModeIndicator fuzzyIndicator = new FuzzyModeIndicator();
        rightTop.add(fuzzyIndicator);
    ...
    private Double applyFuzzyRounding(Double price, int level) {
        ...
        switch (level) {
            case 1: return (double) Math.round(price * 10) / 10.0; // 保留一位小数
            case 2: return (double) Math.round(price); // 个位
            case 3: return (double) (Math.round(price / 10.0) * 10); // 十位
    ...
    // ✨ 全新改造：圆点指示灯风格的下拉菜单触发器
    // ✨ 全新改造：圆点指示灯风格的下拉菜单触发器 (已放大并模仿另外两个类的光晕风格)
    class FuzzyModeIndicator extends JComponent {
        private boolean isHovered = false;

        public FuzzyModeIndicator() {
            setPreferredSize(new Dimension(50, 50));
            setCursor(new Cursor(Cursor.HAND_CURSOR));
            //setToolTipText("模糊比对级别设置");

            // 选项定义
            String[] options = {"精确校对", "保留一位小数", "模糊至个位", "模糊至十位"};

            // 构建弹出菜单
            JPopupMenu popupMenu = new JPopupMenu();
            popupMenu.setBackground(new Color(40, 45, 50)); // 暗色背景
            popupMenu.setBorder(BorderFactory.createLineBorder(new Color(80, 90, 100), 1)); // 边框

            for (int i = 0; i < options.length; i++) {
                JMenuItem item = new JMenuItem(options[i]);
                item.setFont(new Font("微软雅黑", Font.BOLD, 13));
                item.setForeground(Color.WHITE);
                item.setBackground(new Color(40, 45, 50));

                // 悬停变色效果
                item.addChangeListener(e -> {
                    if (item.isArmed()) {
                        item.setBackground(new Color(80, 90, 100));
                    } else {
                        item.setBackground(new Color(40, 45, 50));
                    }
                });

                final int levelIndex = i; // 闭包捕获索引
                item.addActionListener(e -> {
                    fuzzyLevel = levelIndex;
                    System.out.println("当前模糊比对级别切换为: " + options[levelIndex]);
                    repaint(); // 触发重绘，根据状态改变圆点颜色
                });
                popupMenu.add(item);
            }
            ...
            addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    // 在小圆点下方弹出菜单
                    popupMenu.show(FuzzyModeIndicator.this, 0, getHeight());
                }
            });
        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            // 根据不同的模糊级别赋予颜色提示 (新增了配套的光晕色 glowColor)
            Color coreColor;
            Color glowColor;
            switch (fuzzyLevel) {
                case 1:
                    coreColor = new Color(72, 219, 251);
                    glowColor = new Color(72, 219, 251, 80);
                    break; // 保留一位小数 (蓝)
                case 2:
                    coreColor = new Color(254, 202, 87);
                    glowColor = new Color(254, 202, 87, 80);
                    break; // 个位 (黄)
                case 3:
                    coreColor = new Color(255, 107, 107);
                    glowColor = new Color(255, 107, 107, 80);
                    break; // 十位 (红)
                default:
                    coreColor = new Color(130, 130, 130);
                    glowColor = new Color(130, 130, 130, 80);
                    break; // 精确 (灰)
            }

            // 1. 模仿另外两个类，加入常驻的光晕背景层
            ...
            if (isHovered) {
                ...
                g2.fillOval(cx - 21, cy - 21, 42, 42);
            }

            // 3. 核心圆点尺寸放大（由原先的 20x20 放大到 26x26）
            g2.setColor(coreColor);
            g2.fillOval(cx - 13, cy - 13, 26, 26);

            // 4. 内部准星徽章按比例放大并加粗线条
            // 4. 内部徽章：阶梯柱状图 (代表模糊级别递进)
            // 4. 内部徽章：参数调节推子
