...
/**
 * 数据校对助手 CheckerPanel (支持固定格式标包编号的绝对隔离版)
 */
public class CheckerPanel extends BackgroundPanel {
    ...
    private void adjustFontSize() {
        ...
        int dlTitleSize = (int) (22 * scale);
        int dlSubSize = (int) (14 * scale);
        dl.setText("<html><center>" +
                "<span style='font-size:" + dlTitleSize + "px; font-weight:bold; color:white;'>拖入 XLSX 进行隔离校验</span><br><br>" +
                "<span style='font-size:" + dlSubSize + "px; color:#B0B0B0;'>请将 Excel 报价表拖拽至此区域</span>" +
                "</center></html>");
