...
public class MainLauncher extends JFrame { // 定义 MainLauncher 类，继承自 JFrame 主窗口
    ...
    public MainLauncher() { // MainLauncher 类的构造方法
        ...
        int winHeight = prefs.getInt("window_height", 480); // 从本地读取存储的窗口高度，若无则默认为 720
        ...
        try { // 开始捕获可能发生的异常
            java.net.URL iconUrl = getClass().getResource("/orange.png"); // 尝试从资源目录中获取橘子图标的路径
            if (iconUrl != null) { // 如果找到了图片路径
                appIcon = new ImageIcon(iconUrl).getImage(); // 将路径转换为 Image 对象
                setIconImage(appIcon); // 设置当前窗口的任务栏图标
            } else { // 如果没找到图片路径
                System.err.println("没找到图片路径：/orange.png"); // 在控制台输出错误提示
            }
    ...
    private void initSystemTray(Image iconImage) { // 初始化系统托盘的方法
        ...
        TrayIcon trayIcon = new TrayIcon(iconImage, "代理助手 - 后台运行中"); // 创建托盘图标对象并设置提示语
    ...
    class GearButton extends JButton {
        public GearButton() { // 构造方法
            ...
            setToolTipText("系统设置"); // 设置鼠标悬停时的文字提示
    ...
    private JPanel createMainMenu() { // 创建主界面面板的方法
        BackgroundPanel menuPanel = new BackgroundPanel("texture2.png"); // 实例化带背景图的面板
        ...
        // --- 第一排：轻盈冷色（呼应天空与远景，视觉上扬） ---
        // --- 第一排：浓郁冷色（提取自星空与矿物的经典高纯度色彩） ---
// --- 统一全局玻璃质感（黑雾半透玻璃，R, G, B, Alpha） ---
        ...
        JButton btn7 = createModuleButton("ShepherdPanel", "SOFTWARE", glassColor);
    ...
    private JButton createModuleButton(String text, String cardName, Color themeColor) {
        JButton btn = new JButton(text) { // 匿名内部类创建按钮，同时覆写文字
            @Override // 重写按钮的视觉绘制逻辑
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create(); // 拿一把高级画笔
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); // 开启抗锯齿

                boolean isHover = getModel().isRollover(); // 判断鼠标是不是悬停在这个按钮上
                boolean isPressed = getModel().isPressed(); // 判断鼠标是不是按下了这个按钮

                // 动态计算按钮颜色的透明度：按下时220(偏不透明)，悬停时200，普通状态150(比较透明)
                int alpha = isPressed ? 220 : (isHover ? 200 : 150);
                // 动态计算按钮按下的位移像素量：按下时往下挪2个像素，产生物理点击感
                int offset = isPressed ? 2 : 0;

                // 配置画笔为混合后的半透明主题色
                g2.setColor(new Color(themeColor.getRed(), themeColor.getGreen(), themeColor.getBlue(), alpha));
                // 画一个带有 25 像素圆角的实心矩形背景
                g2.fillRoundRect(offset, offset, getWidth() - offset * 2, getHeight() - offset * 2, 25, 25);

                g2.setStroke(new BasicStroke(1.8f)); // 设置 1.8 像素宽的画笔准备画边框
                // 边框设为带透明度的白色，悬停时更白一点制造反光效果
                ...
                g2.setFont(getFont()); // 获取本按钮设定的字体
                FontMetrics fm = g2.getFontMetrics(); // 获取字体的测量工具箱 (用于算宽高)
                // 精准计算出把文字放在居中位置所需的 X 坐标
                int x = (getWidth() - fm.stringWidth(getText())) / 2;
                // 精准计算出把文字放在垂直居中位置所需的 Y 坐标
                int y = (getHeight() + fm.getAscent() - fm.getDescent()) / 2;

                g2.setColor(new Color(0, 0, 0, 60)); // 拿灰黑色画笔
                g2.drawString(getText(), x + 1, y + 1 + offset); // 先画一遍文字，往下偏一点，当做文字阴影
                g2.setColor(Color.WHITE); // 拿纯白画笔
                g2.drawString(getText(), x, y + offset); // 再画一遍真正的文字覆盖上去

                g2.dispose(); // 用完销毁画笔
            }
        };
        btn.setPreferredSize(new Dimension(270, 110)); // 限制按钮大小固定为 270 x 110
        btn.setFont(new Font("微软雅黑", Font.BOLD, 20)); // 设置按钮文字基础字体
        btn.setContentAreaFilled(false); // 禁用系统默认背景填充
        btn.setBorderPainted(false); // 禁用系统默认边框
        btn.setFocusPainted(false); // 禁用系统丑陋的虚线焦点框
        btn.setCursor(new Cursor(Cursor.HAND_CURSOR)); // 鼠标放上去变小手

        btn.addActionListener(e -> switchPanel(cardName)); // 当被点击时，触发上面写的卡片切换逻辑，跳到对应功能页面

        btn.addMouseListener(new MouseAdapter() { // 监听鼠标进出事件来做微动效
            @Override // 鼠标进入按钮瞬间
            public void mouseEntered(MouseEvent e) { btn.setFont(new Font("微软雅黑", Font.BOLD, 22)); } // 把字体放大到 22 制造膨胀效果
            @Override // 鼠标离开按钮瞬间
            public void mouseExited(MouseEvent e) { btn.setFont(new Font("微软雅黑", Font.BOLD, 20)); } // 恢复字体大小
        });
        return btn; // 返回彻底造好的华丽按钮
    }
